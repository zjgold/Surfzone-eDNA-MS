...
input_anacapa_path_1 <- here("data","anacapa_20210516","ucsb_2018_elas_combo","c19_fishcard_taxonomy_tables","Summary_by_percent_confidence","60","c19_fishcard_ASV_raw_taxonomy_60.txt")
input_anacapa_path_2 <- here("data","anacapa_20210516","ucsb_2018_elas_fishcard","fishcard_12S_all_taxonomy_tables","Summary_by_percent_confidence","60","fishcard_12S_all_ASV_raw_taxonomy_60.txt")
input_anacapa_path_3 <- here("data","anacapa_20210516","ucsb_2018_miu_combo","c19_fishcard_taxonomy_tables","Summary_by_percent_confidence","60","c19_fishcard_ASV_raw_taxonomy_60.txt")
input_anacapa_path_4 <- here("data","anacapa_20210516","ucsb_2018_miu_fishcard","fishcard_12S_all_taxonomy_tables","Summary_by_percent_confidence","60","fishcard_12S_all_ASV_raw_taxonomy_60.txt")

input_meta_path <- here("data","metadata_ucsb_2018_032020.txt")
...
saveRDS(barcode_4_use,file= here("decontam","Output_R","barcode_4_use.RDS"))
saveRDS(barcode_2_use,file= here("decontam","Output_R","barcode_2_use.RDS"))
...
saveRDS(ASV.table_long, file=here("decontam","ASV.table_long.RDS"))
saveRDS(ASV.table_used, file=here("decontam","ASV.table_used.RDS"))
...
barcode_4_use <- readRDS(file= here("decontam","Output_R","barcode_4_use.RDS"))
barcode_2_use <-readRDS(file= here("decontam","Output_R","barcode_2_use.RDS"))
metadata <- read.table(input_meta_path, header = 1, sep = "\t", stringsAsFactors = F)
...
ggsave(plot=plot_2,here("decontam","Output_plots","Sample_Read_Depth_Barcode_2.png"), device = "png", width = 12, height = 8, units = "in")
...
ggsave(plot=vc_plot_2,here("decontam","Output_plots","Vector_Contamination_Barcode_2.png"), device = "png", width = 12, height = 8, units = "in")
...
SOM_results_to_keep_filtered <- readRDS("occupancy_results_filtered.RDS")
...
cleaned.tibble.post_occ %>%
  ...
  ungroup() %>% 
  dplyr::select(-Seq_number, -Normalized.reads,-Tot,-sample,-Technical_rep) %>% distinct() %>% 
...
cleaned.tibble.post_occ %>%
  ungroup() %>% 
  dplyr::select(-Seq_number, -Normalized.reads,-Tot,-sample,-Technical_rep) %>% distinct() -> cleaned.tibble.post_occ_v3

saveRDS(cleaned.tibble.post_occ_v3, file ="surf_zone_data.RDS")

cleaned.tibble.post_occ_v2 %>% 
  group_by(sum.taxonomy,Site, Bio_rep,Tech_rep) %>% 
  dplyr::summarise(nReads=sum(nReads)) %>% 
  pivot_wider(names_from="Tech_rep", values_from="nReads", values_fill=0) %>%
  pivot_longer(cols=`1`:`3`,names_to="Tech_rep", values_to="nReads") %>% 
unite("Replicate",Site,Bio_rep,Tech_rep, remove=F) %>% ungroup() %>% 
  filter(., Replicate %in% sample_check$Replicate) %>% 
  filter(sum.taxonomy=="Eukaryota;Chordata;Actinopteri;Atheriniformes;Atherinopsidae;Atherinops;Atherinops affinis") %>% 
  left_join(tot_reads) %>% 
  ggplot(aes(x=tot_reads, y=nReads,color=Bio_rep, shape=Tech_rep)) +geom_point() +facet_wrap(.~Site)
...
holder_1 %>% 
 ...
 ggsave(plot=plot_prop2,here("species_prop_plot2.png"), device = "png", width = 12, height = 8, units = "in", limitsize = FALSE)
 ...
 holder_1 %>% 
   ...
   mutate(read_depth = case_when(tot_reads > 100000 ~">100K",
                                 tot_reads >= 50000  ~"50K-100K",
                                 TRUE ~"<50K")) -> zero_inf_prop_data
 
 zero_inf_prop_data %>% 
   group_by(name, zero_inf) %>% 
   count() %>% 
   pivot_wider(values_from = "n", names_from="zero_inf", values_fill=0) %>% 
   filter(., `Three reps above zero` ==0) -> species_with_only_zero_reps
 
 zero_inf_prop_data %>% 
   group_by(name, zero_inf) %>% 
   count() %>% 
   pivot_wider(values_from = "n", names_from="zero_inf", values_fill=0) -> zero_prop_Data
 
 zero_inf_nReads_data %>% 
   group_by(name) %>% 
   dplyr::summarise(tot_reads = sum(nReads), mean_reads=mean(nReads)) -> nReads_species
 
 zero_prop_Data %>% 
   mutate(., Type= case_when(`Three reps above zero` > 0 & `Three reps above zero` > `One rep is zero`~ "More Triplicates > 0 than Triplicates w/ 0",
                             `Three reps above zero` > 0 ~ "Fewer Triplicates > 0 than Triplicates w/ 0",
                             TRUE ~ "No Triplicates > 0")) %>% 
   pivot_longer(cols=`One rep is zero`:`Three reps above zero`, values_to = "count", names_to="zero_inf") %>% 
   left_join(nReads_species) %>% 
   ggplot(aes(color=name, y=count,x=log10(tot_reads), shape=zero_inf))+geom_point() + ylab("Count of Triplicate Types") +xlab("log(Total Reads per Species)") +
   facet_wrap(Type~.)-> plot_count_vs_tot_reads
 
 ggsave(plot=plot_count_vs_tot_reads,here("plot_count_vs_tot_reads.png"), device = "png", width = 24, height = 8, units = "in", limitsize = FALSE)
 
 zero_inf_prop_data %>% 
   mutate(., species_type = if_else(name %in% species_with_only_zero_reps$name,"Low performing species","High performing species")) %>% 
   filter(., species_type!="Low performing species") %>% 
   group_by(name, zero_inf,read_depth,species_type) %>% 
   count() %>% 
   ggplot(., aes(x=zero_inf, y=n), color=name) +facet_wrap(read_depth~.) +geom_boxplot()
 
 zero_inf_prop_data %>% 
   filter(., name %in% species_with_only_zero_reps$name) %>% View()
 ...
 cleaned.tibble.post_occ_v2 %>% 
   ...
   unite("Replicate",Site,Bio_rep,Tech_rep, remove=F) %>% ungroup() %>% 
   filter(., Replicate %in% sample_check$Replicate) %>% 
   left_join(tot_reads) -> holder_2
 
 holder_2 %>% 
   group_by(sum.taxonomy,Site, Bio_rep) %>% 
   dplyr::summarise(mean_nReads = mean(nReads),
                    max_nReads=max(nReads),
                    min_nReads=min(nReads)) ->  avg_data_nReads
 
 holder_2 %>% 
   left_join(avg_data_nReads) %>% 
   mutate(zero_inf = case_when(min_nReads > 0 & max_nReads > 0 ~"Three reps above zero",
                               min_nReads == 0 & max_nReads > 0 ~"One rep is zero",
                               min_nReads == 0 & max_nReads == 0 ~"All zero")) %>% 
 ggplot(aes(x=tot_reads, y=nReads, color=zero_inf, shape=Bio_rep)) +geom_point() +facet_wrap(sum.taxonomy~., scales = "free") -> plot_nReads
 
 ggsave(plot=plot_nReads,here("species_free_plot.png"), device = "png", width = 30, height = 30, units = "in")
 ...
 holder_2 %>% 
  ...
  holder_2 %>% 
    ...
    dplyr::mutate(name = ifelse(is.na(Family) | Family == "", Order, name)) %>%
    filter(., !is.na(name)) %>%  
    group_by(name,Site, Bio_rep) %>% 
    dplyr::summarise(mean_nReads = mean(nReads),
                     sd_nReads = sd(nReads),
                     max_nReads=max(nReads),
                     min_nReads=min(nReads)) %>% 
    filter(., sd_nReads > 0) %>% 
    mutate(.,cv_nReads=sd_nReads/mean_nReads) %>% 
    ggplot(aes(x=mean_nReads, y=cv_nReads)) +geom_point()
  
  
  cleaned.tibble.post_occ_v2 %>% 
    group_by(seq_number,Site, Bio_rep,Tech_rep) %>% 
    dplyr::summarise(nReads=sum(nReads)) %>% 
    unite("Replicate",Site,Bio_rep,Tech_rep, remove=F) %>% 
    dplyr::select(-Replicate) %>% 
    pivot_wider(names_from="Tech_rep", values_from="nReads", values_fill=0) %>%
    ...
    unite("Replicate",Site,Bio_rep,Tech_rep, remove=F) %>% ungroup() %>% 
    filter(., Replicate %in% sample_check$Replicate) %>% 
    left_join(tot_reads) -> holder_3
  
  holder_3 %>%  
    group_by(seq_number,Site, Bio_rep) %>% 
    dplyr::summarise(mean_nReads = mean(nReads),
                     sd_nReads = sd(nReads),
                     max_nReads=max(nReads),
                     min_nReads=min(nReads)) %>% 
...
cleaned.tibble.post_occ_v2 %>% 
  ...
  filter(., Replicate %in% sample_check$Replicate) %>% 
  left_join(tot_reads) %>%  
  group_by(Site ,   Bio_rep, sum.taxonomy) %>% 
