...
input_anacapa_path_1 <- here("analysis","data","anacapa","elas_2018_fishcard_12S_all_taxonomy_tables","Summary_by_percent_confidence","60","fishcard_12S_all_ASV_raw_taxonomy_60_edited.txt")
input_anacapa_path_2 <- here("analysis","data","anacapa","miu_elas_2018_fishcard_12S_all_taxonomy_tables","Summary_by_percent_confidence","60","fishcard_12S_all_ASV_raw_taxonomy_60_edited.txt")
input_meta_path <- "/Volumes/GoogleDrive/Shared drives/eDNA 2017&2018/2018_analysis/analysis/data/metadata_ucsb_2018_032020.txt"
...
saveRDS(ASV.table_long, file="ASV.table_long.RDS")
saveRDS(ASV.table_used, file="ASV.table_used.RDS")


# Technical Replicate Exploration -------
ASV.table_used_mk <- readRDS(file=here("analysis","decontam","ASV.table_used.RDS"))
...
asv_means %>% 
  left_join(hash.key) %>% 
  filter(., str_detect(sum.taxonomy,"Engraulis")) %>% 
  ggplot(., aes(x=Bio_rep, y=mean_prop, color=seq_number)) + geom_point() + facet_wrap(~Site) +
  theme(axis.text.x = element_text(angle = 90))+ 
  geom_errorbar(aes(ymin=mean_prop-sd_prop, ymax=mean_prop+sd_prop), width=.2,position=position_dodge(.9)) +
  theme(legend.position = "none")
...
asv_means %>% 
  ...
  filter(., max_prop >0.01) %>%
  filter(., sd_prop > 0) %>% 
  ggplot(., aes(x=sum.taxonomy, y=sd_prop/mean_prop, color=seq_number)) + geom_boxplot() +
